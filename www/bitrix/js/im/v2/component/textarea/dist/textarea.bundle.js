...
(function (exports,ui_iconSet_outline,ui_uploader_core,im_v2_provider_service_message,im_v2_lib_soundNotification,im_v2_lib_inputAction,im_v2_lib_escManager,im_v2_lib_message,im_v2_lib_desktopApi,im_v2_component_elements_tabs,im_v2_lib_helpdesk,ui_system_chip_vue,im_v2_lib_localStorage,im_v2_component_elements_pulseAnimation,im_v2_lib_smileManager,im_v2_provider_service_sticker,main_polyfill_intersectionobserver,main_core_events,im_v2_lib_menu,im_v2_provider_service_sending,im_v2_component_sticker,im_v2_lib_promo,calendar_sharing_interface,vote_application,im_v2_component_elements_menu,im_v2_lib_entityCreator,im_v2_lib_analytics,im_v2_lib_permission,im_v2_lib_notifier,file_dialog,im_v2_model,im_v2_lib_draft,im_v2_lib_hotkey,im_v2_provider_service_uploading,im_v2_component_elements_mediaGallery,im_v2_component_elements_sendButton,ui_icons,im_v2_lib_feature,im_v2_lib_copilot,im_v2_lib_channel,im_v2_lib_user,im_v2_lib_logger,im_v2_lib_search,im_v2_lib_rest,im_v2_component_elements_scrollWithGradient,im_v2_component_elements_avatar,im_v2_component_elements_chatTitle,im_v2_lib_textHighlighter,im_v2_application_core,im_v2_lib_parser,ui_vue3_components_richLoc,im_v2_component_elements_loader,im_v2_lib_market,im_v2_component_elements_autoDelete,im_v2_provider_service_chat,im_v2_lib_autoDelete,im_v2_const,main_core,main_popup,im_v2_lib_textarea,im_v2_component_elements_popup,im_v2_lib_quote,ui_system_input_vue,ui_iconSet_api_vue,im_v2_lib_utils) {
	'use strict';
	...
	function _calculateActivePack2() {
	  ...
	  this.emit(ObserverManager.CHANGE_ACTIVE_PACK_EVENT, {
	    id: bestPack.id,
	    type: bestPack.type
	  });
	}
	...
	ObserverManager.CHANGE_ACTIVE_PACK_EVENT = 'changeActivePack';
	...
	const Pack = {
	  ...
	  computed: {
	    ...
	    canShowContextMenu() {
	      return this.pack.type === im_v2_const.StickerPackType.custom || this.isRecentPack;
	    },
	    isRecentPack() {
	  ...
	  methods: {
	    ...
	    openPackMenu(event) {
	      ...
	      (_PopupManager$getPopu = main_popup.PopupManager.getPopupById(im_v2_const.PopupType.stickerPackContextMenu)) == null ? void 0 : _PopupManager$getPopu.close();
	    ...
	    openStickerMenu({
	      ...
	      (_PopupManager$getPopu2 = main_popup.PopupManager.getPopupById(im_v2_const.PopupType.stickerContextMenu)) == null ? void 0 : _PopupManager$getPopu2.close();
	      if (!this.stickerMenu) {
	...
	const HeaderTabs = {
	  ...
	  watch: {
	    activePack(newPack) {
	      const activePackIndex = this.packs.findIndex(pack => pack.id === newPack.id && pack.type === newPack.type);
	...
	const TabStickers = {
	  ...
	  data() {
	    return {
	      activePack: {
	        id: im_v2_const.RecentPackId,
	        type: im_v2_const.RecentPackType
	      },
	  ...
	  computed: {
	    ...
	    recentPack() {
	      return {
	        id: im_v2_const.RecentPackId,
	        key: `${im_v2_const.RecentPackId}:${im_v2_const.RecentPackType}`,
	        type: im_v2_const.RecentPackType,
	...
	const EmotePopup = {
	  ...
	  methods: {
	    getInitialTab() {
	      return im_v2_lib_localStorage.LocalStorageManager.getInstance().get(im_v2_const.LocalStorageKey.emotePopupTab, TabType$1.emoji);
	    },
	    loc(phraseCode) {
	      return this.$Bitrix.Loc.getMessage(phraseCode);
	    },
	  ...
	  computed: {
	    ...
	    toolbarItems() {
	      return [{
	        ...
	        handler: () => this.applyDecoration('KeyU')
	      }, {
	        ...
	        handler: () => this.applyDecoration('KeyS')
	      }, {
	        ...
	        handler: () => this.applyDecoration('code')
	      }];
	    }
	  },
	  methods: {
	...
	const ChatTextarea = {
	  ...
	  computed: {
	    ...
	    isFormatToolbarAvailable() {
	      const isFeatureEnabled = im_v2_lib_feature.FeatureManager.isFeatureAvailable(im_v2_lib_feature.Feature.isTextFormatToolbarAvailable);
	      return isFeatureEnabled && this.showFormatToolbar;
	    }
	  },
	  watch: {
	    text(newValue) {
