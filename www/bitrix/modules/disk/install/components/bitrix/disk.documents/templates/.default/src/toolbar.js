...
export default class Toolbar
{
	...
	static runCreating(documentType, service, nodeToBind: ?HTMLElement, nodeName): void
	{
	...
	static createDocx(service, element, nodeName)
	{
		const code = this.resolveServiceCode(service);
		if (code)
		{
			const nodeToBind = this.resolveNodeToBind(element);
			this.runCreating('docx', code, nodeToBind, nodeName);
		}
	}

	static createXlsx(service, element, nodeName)
	{
		const code = this.resolveServiceCode(service);
		if (code)
		{
			const nodeToBind = this.resolveNodeToBind(element);
			this.runCreating('xlsx', code, nodeToBind, nodeName);
		}
	}

	static createPptx(service, element, nodeName)
	{
		const code = this.resolveServiceCode(service);
		if (code)
		{
			const nodeToBind = this.resolveNodeToBind(element);
			this.runCreating('pptx', code, nodeToBind, nodeName);
		}
	}
