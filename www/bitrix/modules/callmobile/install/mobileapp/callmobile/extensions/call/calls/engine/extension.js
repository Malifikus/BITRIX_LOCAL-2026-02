...
jn.define('call/calls/engine', (require, exports, module) => {
	...
	class CallEngine
	{
		constructor()
		{
			...
			this.isInitializingCallFromPush = false;
		...
		onActiveCallsReceived(activeCalls)
		{
			...
			const tryToRemoveCall = (call) => {
				...
				if (call.id in callsToInstantiate || call.createdFromPush)
				{
		...
		startWithPush()
		{
			...
			this.isInitializingCallFromPush = true;
			...
			{
				const call = this._instantiateCall(pushParams.PARAMS.call, null, null, null, null, true);
		...
		_onNativeIncomingCall(nativeCall)
		{
			...
			this._instantiateCall(nativeCall.params.call, nativeCall.params.connectionData, nativeCall.params.users, nativeCall.params.logToken, nativeCall.params.userData, true);
			BX.postComponentEvent('CallEvents::incomingCall', [{
				callId,
				callUuid,
				video: isVideo,
		...
		getLegacyCallWithId(id, createdFromPush = false)
		{
			return new Promise((resolve, reject) => {
				...
				this.getRestClient().callMethod(ajaxActions.getCall, { callId: id }).then((response) => {
					...
					resolve({
						call: this._instantiateCall(data.call, data.connectionData, data.users, data.logToken, data.userData, createdFromPush),
						isNew: false,
					});
				}).catch((error) => {
		...
		_onPullIncomingCall(params, extra)
		{
			...
			if (!call && !this.isInitializingCallFromPush)
			{
			...
			if (call && !call.createdFromPush && !(call instanceof CallStub))
			{
				if (isLegacyCall)
				{
		...
		_onUnknownCallPing(callId, serverTimeAgo, ttl, createdFromPush = false)
		{
			return new Promise((resolve, reject) => {
				...
				this.getLegacyCallWithId(callId, createdFromPush).then((result) => {
		...
		_instantiateCall(callFields, connectionData, users, logToken, userData, createdFromPush = false)
		{
			const callId = callFields.ID || callFields.id;
			const callUuid = callFields.UUID || callFields.uuid;
			...
			call = callFactory.createCall({
				...
				createdFromPush: createdFromPush,
			});
