...
jn.define('im/messenger/lib/element/recent/item/chat/channel', (require, exports, module) => {
	...
	class ChannelItem extends ChatItem
	{
		...
		initParams(modelItem, options)
		{
			...
			let channelCounters = 0;
			if (Feature.isMessengerV2Enabled)
			{
				channelCounters = store.getters['counterModel/getNumberChildCounters'](this.dialog?.chatId);
			}
			else
			{
				channelCounters = store.getters['commentModel/getChannelCounters'](this.dialog?.chatId);
			}
